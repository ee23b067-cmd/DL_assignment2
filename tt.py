...
def get_device():
    ...
    if major < 7:
        print(
            f"CUDA device sm_{major}{minor} is not supported by this PyTorch build. "
            "Falling back to CPU."
        )
        return torch.device("cpu")
...
# DATA LOADER
# -----------------------------
def get_dataloader(root_dir, batch_size, split="train"):
    dataset = OxfordIIITPetDataset(root=root_dir, split=split, transform=None, target_size=(224, 224))
...
def train_classifier(root_dir, epochs=25, batch_size=32, lr=3e-4):
    ...
    train_loader = get_dataloader(root_dir, batch_size, split="train")
    val_loader = get_dataloader(root_dir, batch_size, split="test")
